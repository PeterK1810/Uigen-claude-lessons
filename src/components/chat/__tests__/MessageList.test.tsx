...
// Mock the ToolCallStatus component
vi.mock("../ToolCallStatus", () => ({
  ToolCallStatus: ({ toolInvocation }: any) => (
    <div data-testid="tool-call-status">{toolInvocation.toolName}</div>
  ),
}));
...
test("MessageList renders messages with parts", () => {
  ...
  expect(screen.getByTestId("tool-call-status")).toBeDefined();
});
